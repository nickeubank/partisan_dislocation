...
class TestPartisanDislocation(unittest.TestCase):
    ...
    def test_random_points_in_polygon_uniformswing(self):
        df = gpd.GeoDataFrame(
            {
                "dem": [1, 1, 0, 0, 0, 0],
                "rep": [0, 0, 1, 1, 1, 1],
        ...
        result = random_points_in_polygon(df, p=1, uniform_swing_to_dems=1 / 6)
        benchmark = pd.Series([1, 1, 1, 0, 0, 0], name="dem")
        pd.testing.assert_series_equal(result["dem"], benchmark)
